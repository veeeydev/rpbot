...
//if statements 
if (code.includes("$if[")) { 
    for (let statement of code.split("$if[").slice(1).reverse()) {
        const r = code.split("$if[").length - 1 
        
        if (!code.includes("$endif")) return message.channel.send(`:x: $if is missing $endif`) 
        
        const everything = code.split("$if[")[r].split("$endif")[0] 
        
        statement = code.split("$if[")[r].split("$endif")[0]
        ...
        const elseIfAction = statement.includes("$elseIf") 
        
        const elseIfs = {}  
        
        if (elseIfAction) {
            for (const data of statement.split("$elseIf[").slice(1)) {
                if (!data.includes("$endelseIf")) return message.channel.send(`❌ $elseIf is missing $endelseIf!`)
                
                const inside = data.split("$endelseIf")[0] 
                ...
                statement = statement.replace(`$elseIf[${inside}$endelseIf`, "") 
            }
        } 
        
        const elseAction = statement.includes("$else") 
        
        const ifCode = elseAction ? statement.split("\n").slice(1).join("\n") .split("$else")[0] : statement.split("\n").slice(1).join("\n").split("$endif")[0]
        
        const elseCode = elseAction ? statement.split("$else")[1].split("$endif")[0] : "" 
