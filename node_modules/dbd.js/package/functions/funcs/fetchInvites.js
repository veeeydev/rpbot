module.exports = async d => {
    ...
    const inside = code.split("$fetchInvites[")[r].split("]")[0] 
    ...
    for (const invite of invites.array()) {
        for (const command of inside.split(";")) {
            ...
            if (!cmd) return d.error(`❌ Invalid command ${command} in \`$fetchInvites[${inside}]\``) 
            
            d.interpreter(d.client, d.message, d.args, cmd, undefined, undefined, undefined, {
                invite: invite 
            })
        }
    }
