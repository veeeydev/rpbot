...
module.exports = async d => {
    ...
    const r = code.split("$globalCooldown[").length - 1

    const inside = code.split("$globalCooldown[")[r].split("]")[0]
    ...
    if (!ms(time)) return d.error(`:x: Invalid time '${time}' in \`$globalCooldown[${inside}]\``)

    /*const item = await db.fetch(`${d.command.name}_cooldown_${d.message.author.id}`) 

    if (item && ms(time) - (Date.now() - item) > 999) {
        return error(d, errorMessage.split("%time%").join(toParse(Object.entries(parse(ms(time) - (Date.now() - item))).map((x, y) => {
            if (x[1] > 0 && y < 4) return `${x[1]}${x[0][0]}`
            else return undefined
        }).filter(e => e).join("")).string))
    } else {
        db.set(`${d.command.name}_cooldown_${d.message.author.id}`, Date.now())
    }*/

  const item = await d.client.db.findOne({
    variable: `${d.command.name}`,
    varID: `cooldowns_${d.message.author.id}`,
})
