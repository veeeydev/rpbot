module.exports = async d => {
    ...
    const inside = code.split("$deleteMessage[")[r].split("]")[0] 
    ...
    if (!channel) return d.error(`❌ Invalid channel ID in \`$deleteMessage[${inside}]\``) 
    
    const m = await channel.messages.fetch(messageID).catch(err => null) 
    
    if (!m) return d.error(`❌ Invalid message ID in \`$deleteMessage[${inside}]\``) 
